...
gulp.task('revision', function () {
  return gulp.src([gulp.sources.dist + '/styles/*.css'], {base: gulp.sources.dist})
    .pipe(rev())
    .pipe(gulp.dest(gulp.sources.dist))
    .pipe(rev.manifest())
    .pipe(gulp.dest('.tmp'));
...
// Build source
gulp.task('build', () => {
  runSequence('clean', 'fileinclude', 'htmlhint', 'sass', 'minify', 'copy:fonts', 'prettify', 'cleanFiles', (e) => {
    if (!e) {
      console.log('Success!');
    }
  });
